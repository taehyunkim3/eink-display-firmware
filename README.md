...
PlatformIO 기반 ESP32 펌웨어입니다. `eink-frontend`의 `/api/screen.bin` 1bpp 비트맵을 받아 800 x 480 e-ink 화면에 출력합니다.
...
```cpp
#define DEVICE_ENDPOINT "http://192.168.0.10:3000/api/screen.bin"
```
...
2. `curl`로 비트맵 API가 200을 주는지 확인합니다.
   ```bash
   curl -H "Authorization: Bearer <token>" -I https://your-app.vercel.app/api/screen.bin
   ```
...
- 비트맵 크기: Serial Monitor에 찍히는 `Bitmap content length`와 `Bitmap received bytes`가 `48000`인지 확인합니다.
