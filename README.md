...
PlatformIO 기반 ESP32 펌웨어입니다. `mrdevello-food`의 `/api/eink-summary` 데이터를 직접 받아 800 x 480 e-ink 화면에 간식 투표 현황과 참여 QR 코드를 그립니다.
...
#define DEVICE_AUTH_TOKEN "mrdevello-food의 EINK_SUMMARY_TOKEN과 같은 값"
```

로컬 Next.js 서버로 테스트할 때는 `localhost`가 아니라 Mac의 같은 Wi-Fi LAN IP를 써야 합니다.

```cpp
#define DEVICE_ENDPOINT "http://192.168.0.10:3000/api/eink-summary"
```
...
1. `mrdevello-food`를 Vercel에 배포하거나 로컬에서 실행합니다.
2. `curl`로 JSON API가 200을 주는지 확인합니다.
   ```bash
   curl -H "Authorization: Bearer <token>" https://mrdevello-food.vercel.app/api/eink-summary
   ```
3. `include/config.h`에 같은 URL과 토큰을 넣습니다.
4. USB로 보드를 연결하고 업로드합니다.
5. 화면에서 라운드, 상품별 득표수, 배송 정보, 참여 QR 코드를 확인합니다. 실패하면 Serial Monitor의 `JSON HTTP`, `JSON 다운로드 실패`, `JSON 파싱 실패` 로그를 확인합니다.
...
Seeed 회로도에는 `KEY0=GPIO3`, `KEY1=GPIO4`, `KEY2=GPIO5`로 표시됩니다. 이 프로젝트의 `left/right/refresh` 이름은 회로도 배치가 아니라 기기를 정면에서 봤을 때의 실제 물리 위치 기준입니다.
...
reTerminal E1001의 microSD는 FAT32, 최대 32GB를 사용합니다. 펌웨어는 마지막 정상 JSON을 `/eink/cache`에 보관해 네트워크 장애 때 다시 표시합니다. OTA 이미지는 `/eink/update`에 임시 저장하며, 수동 업데이트는 `/eink/update/manual.bin`을 사용합니다.

SD 카드가 없어도 API 호출과 화면 표시는 동작합니다. 오프라인 캐시와 SD 기반 업데이트만 사용할 수 없습니다.
...
- `DEVICE_AUTH_TOKEN`: `mrdevello-food`의 `EINK_SUMMARY_TOKEN`과 정확히 같아야 합니다.
